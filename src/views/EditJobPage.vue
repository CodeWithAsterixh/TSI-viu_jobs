...
import { areValuesEqual } from '@/libs/__objectMethods'
...
const editTries = ref(0)

const form: jobType = reactive({
  type: 'Full-Time',
  title: '',
  description: '',
  salary: '',
...
const state = reactive<{ isLoading: boolean; job?: jobType }>({
  isLoading: true,
})
...
const handleSubmit = async () => {
  const updatedJob = JSON.parse(JSON.stringify(form))
  const originJobObj = JSON.parse(JSON.stringify(state.job))
  const maxTries = 5
  editTries.value++
  const triesLeft = maxTries - editTries.value

  const isEqual = areValuesEqual(originJobObj, updatedJob)

  if (!isEqual) {
    db.update(updatedJob)
    toast.success('Job Updated Successfully')
    router.push(`/jobs/${jobId}`)
  } else {
    if (triesLeft > 0) {
      toast.info(
        `nothing changed! please edit some more information\nyou have ${triesLeft} tries left`,
      )
    } else {
      toast.info(`oops you've made some much attempt, please try again later`)
      router.push(`/jobs/${jobId}`)
    }
  }
}
